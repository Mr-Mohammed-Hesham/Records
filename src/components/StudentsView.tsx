...
export const StudentsView: React.FC<StudentsViewProps> = ({
  ...
  // Pre-calculate stats for all students
  const studentsWithStats = useMemo(() => {
    return students.map(student => {
      const studentResults = allResults.filter(r => r.studentDocId === student.id);
  ...
  }, [students, allResults, settings.gradingScale]);
  ...
  const filteredStudents = useMemo(() => {
    return studentsWithStats.filter(({ student, stats }) => {
      ...
      const matchSearch =
        ...
        (student.parentPhone && student.parentPhone.includes(searchLower));
      ...
      // Subject
      if (subjectFilter !== 'الكل' && student.subject !== subjectFilter) return false;
  ...
  }, [studentsWithStats, searchTerm, gradeFilter, groupFilter, subjectFilter, statusFilter]);
  ...
  // Unique groups in students list
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-lg border transition-colors cursor-pointer inline-flex items-center gap-1 ${
                sortField === 'name' ? 'bg-indigo-50 border-indigo-200 text-indigo-700 font-bold' : 'bg-slate-50 border-slate-200'
              }`}
            >
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-lg border transition-colors cursor-pointer inline-flex items-center gap-1 ${
                sortField === 'average' ? 'bg-indigo-50 border-indigo-200 text-indigo-700 font-bold' : 'bg-slate-50 border-slate-200'
              }`}
            >
              متوسط الدرجات {sortField === 'average' && (sortDirection === 'asc' ? '↑' : '↓')}
            </button>
            <button
              onClick={() => toggleSort('examsCount')}
              className={`px-2.5 py-1 rounded-lg border transition-colors cursor-pointer inline-flex items-center gap-1 ${
                sortField === 'examsCount' ? 'bg-indigo-50 border-indigo-200 text-indigo-700 font-bold' : 'bg-slate-50 border-slate-200'
              }`}
            >
              عدد الامتحانات {sortField === 'examsCount' && (sortDirection === 'asc' ? '↑' : '↓')}
            </button>
          </div>
          ...
          <table className="w-full text-right text-xs">
            <thead className="bg-slate-50/80 text-slate-700 font-bold border-b border-slate-200">
              <tr>
                ...
                <th className="py-3.5 px-3 text-center">حالة الطالب</th>
                  ...
                  <td colSpan={11} className="py-12 text-center text-slate-400">
                    {students.length === 0 ? (
                      <div className="space-y-3">
                        ...
                        <p className="font-semibold text-slate-600">لا يوجد طلاب مسجلين حتى الآن</p>
                        <p className="text-xs text-slate-400">ابدأ بإضافة أول طالب أو استيراد البيانات من الإعدادات</p>
                        <button
                          onClick={onAddStudent}
